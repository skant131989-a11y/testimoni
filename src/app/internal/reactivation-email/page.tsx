...
export default function WelcomeEmailPreviewPage() {
  ...
  const previewSrc = useMemo(() => {
    ...
    return `/internal/reactivation-email/preview?${params.toString()}`;
  }, [email, previewNonce]);
  ...
  const fetchMeta = useCallback(async () => {
    ...
    const res = await fetch(`/internal/reactivation-email/preview?${params.toString()}`);
  ...
  const subject = meta?.recipient.name
    ? `${meta.recipient.name.split(" ")[0]}, want your first testimonial live in 30 seconds?`
    : "Want your first testimonial live in 30 seconds?";

  return (
    <div className="mx-auto max-w-6xl p-6">
      <h1 className="text-2xl font-bold">Reactivation email preview</h1>
      <p className="mt-1 text-sm text-muted-foreground">
        Internal tool. Enter an email of a user whose workspace has 0
        testimonials so far — we&apos;ll pull their name, workspace,
        widget, and form from the DB and render the email personalised
        for their account.
      </p>
